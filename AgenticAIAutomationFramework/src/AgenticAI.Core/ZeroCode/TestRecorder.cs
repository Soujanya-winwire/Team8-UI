        ...
        private async Task SetupActionListeners()
        {
            ...
            await _page.ExposeFunctionAsync("__playwrightRecordAction", (JsonElement eventData) =>
            {
                try
                {
                    ...
                    // CONFIGURABLE AUTO-WAIT: Add wait after every recorded action based on configuration
                    var config = Configuration.ConfigurationManager.Instance.FrameworkConfig;
                    int waitTimeMs = config.RecorderWaitTimeMs;
                    
                    if (waitTimeMs > 0 && actionType.ToLower() != "wait")
                    {
                        double waitSeconds = waitTimeMs / 1000.0;
                        var waitAction = new RecordedAction
                        {
                            ActionType = "Wait",
                            Locator = "",
                            Value = waitSeconds.ToString("0.###"), // Format to remove trailing zeros
                            Description = $"Wait {waitSeconds}s after {actionType} (configured delay)",
                            Timestamp = _recordedActions.Count,
                            Metadata = new Dictionary<string, string>
                            {
                                ["AutoGenerated"] = "true",
                                ["Reason"] = "Configured recorder wait time",
                                ["TriggerAction"] = actionType,
                                ["ConfiguredMs"] = waitTimeMs.ToString()
                            }
                        };
                        _recordedActions.Add(waitAction);
                        Logger.Info($"⏱️ Auto-added {waitSeconds}s wait (configured: {waitTimeMs}ms)");
                        OnActionCaptured?.Invoke(waitAction);
                    }
...
");
        ...
        {
            // Use the comprehensive ParameterResolver instead of custom logic
            var inferredName = DataDriven.ParameterResolver.InferParameterName(selector, "Type");
            
            // If inference succeeded, use it; otherwise fall back to "value"
            return !string.IsNullOrWhiteSpace(inferredName) ? inferredName : "value";
        }
