...
// Global object to track which scenarios have auto-waits visible (per scenario)
let scenarioAutoWaitVisibility = {};

/**
 * Get unique key for current scenario
 */
function getScenarioKey() {
    if (!currentEditingScenario) return null;
    return `${currentEditingScenario.module}::${currentEditingScenario.name}`;
}

/**
 * Check if auto-waits should be shown for current scenario
 */
function shouldShowAutoWaits() {
    const key = getScenarioKey();
    return key ? (scenarioAutoWaitVisibility[key] === true) : false;
}
...
function renderCompactStepsList() {
    ...
    // Filter out auto-generated Wait actions for display (unless user wants to see them for this scenario)
    const showAutoWaits = shouldShowAutoWaits();
    ...
    if (visibleActions.length === 0 && assertions.length === 0) {
        return `
    ...
    // Count auto-generated waits (always, regardless of visibility)
    const autoWaitCount = actions.filter(action => {
        if (action.actionType === 'Wait' || action.ActionType === 'Wait') {
            const meta = action.metadata || action.Metadata;
            return meta && (meta.AutoGenerated === 'true' || meta.AutoGenerated === true);
        }
        return false;
    }).length;
    
    // Actions Table
    if (visibleActions.length > 0 || (actions.length > 0 && showAutoWaits)) {
        // Count total steps (visible actions + assertions)
        const totalSteps = visibleActions.length + assertions.length;
        ...
        html += `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; flex-wrap: wrap; gap: 8px;">
                <h4 style="margin: 0; color: #1f2937; font-size: 14px; display: flex; align-items: center; gap: 6px;">
                    ...
                    Scenario Flow (${totalSteps} steps: ${visibleActions.length} actions, ${assertions.length} assertions)
                    ${autoWaitCount > 0 && !showAutoWaits ? `<span style="color: #6b7280; font-size: 11px; font-weight: normal;">(${autoWaitCount} auto-waits hidden)</span>` : ''}
                    ${autoWaitCount > 0 && showAutoWaits ? `<span style="color: #ca8a04; font-size: 11px; font-weight: normal;">(${autoWaitCount} auto-waits shown)</span>` : ''}
                </h4>
                <div style="display: flex; gap: 8px; align-items: center;">
                    ${autoWaitCount > 0 ? `
                        <label style="display: flex; align-items: center; gap: 6px; font-size: 12px; color: #6b7280; cursor: pointer; user-select: none;">
                            <input type="checkbox" 
                                   id="toggleAutoWaits" 
                                   ${showAutoWaits ? 'checked' : ''} 
                                   onchange="toggleAutoWaits()"
                                   style="cursor: pointer;">
                            <span>${showAutoWaits ? 'Hide' : 'Show'} auto-waits</span>
                        </label>
                    ` : ''}
        ...
        visibleActions.forEach((action, visibleIdx) => {
            // Find the original index in the full actions array
            ...
            // Check if this is an auto-generated wait
            const meta = action.metadata || action.Metadata;
            const isAutoWait = (action.actionType === 'Wait' || action.ActionType === 'Wait') && 
                              meta && (meta.AutoGenerated === 'true' || meta.AutoGenerated === true);
            
            // Render the ACTION row - SECOND
            html += `
                <tr style="border-bottom: 1px solid #f3f4f6; ${isAutoWait ? 'background: #fef9c3; opacity: 0.85;' : ''}">
                    <td style="padding: 6px 10px; color: ${isAutoWait ? '#ca8a04' : '#3b82f6'}; font-weight: 600; font-size: 11px;">
                        ${isAutoWait ? '<i class="fas fa-robot" style="font-size: 10px; margin-right: 3px;"></i>' : ''}${visibleIdx + 1}
                    </td>
                    <td style="padding: 6px 10px;">
                        <span class="badge ${isAutoWait ? '' : 'badge-primary'}" style="font-size: 10px; padding: 3px 8px; ${isAutoWait ? 'background: #fbbf24; color: #78350f;' : ''}">${escapeHtml(action.actionType)}</span>
                        ${isAutoWait ? '<span style="font-size: 9px; color: #92400e; margin-left: 4px;">(auto)</span>' : ''}
                    </td>
                    ...
                    <td style="padding: 6px 10px; color: #6b7280; font-size: 11px; word-break: break-all; line-height: 1.3;">
                        ${(() => {
                            ...
                            // Show auto-wait with helpful note
                            if (isAutoWait) {
                                return `<span style="color: #ca8a04; font-weight: 600;">${escapeHtml(action.value)}s</span>` +
                                       `<span style="color: #92400e; font-size: 9px; margin-left: 4px;">(editable)</span>`;
                            }
...
function renderStepsList() {
    ...
    // Filter out auto-generated Wait actions (from recorder configuration) - unless user wants to see them for this scenario
    const showAutoWaits = shouldShowAutoWaits();
    const visibleActions = actions.filter(action => {
        if (!showAutoWaits) {
            // Hide Wait actions that are auto-generated (case-insensitive check for both metadata and Metadata)
            if (action.actionType === 'Wait' || action.ActionType === 'Wait') {
                const meta = action.metadata || action.Metadata;
                if (meta && (meta.AutoGenerated === 'true' || meta.AutoGenerated === true || meta.autoGenerated === 'true')) {
                    return false;
                }
            }
        }
    ...
    // 1. Render all VISIBLE actions (excluding auto-generated waits)
    visibleActions.forEach((action, actionIndex) => {
        // Find the original index in the full actions array
        const originalIndex = actions.indexOf(action);
        html += renderStepItem(action, originalIndex, 'action');
    });
...
/**
 * Toggle visibility of auto-generated wait actions for current scenario
 */
function toggleAutoWaits() {
    const key = getScenarioKey();
    if (!key) return;
    
    // Toggle the visibility state for this specific scenario
    scenarioAutoWaitVisibility[key] = !scenarioAutoWaitVisibility[key];
    
    console.log('[Scenario Editor] Toggle auto-waits for', key, ':', scenarioAutoWaitVisibility[key]);
    renderScenarioModal();
}
